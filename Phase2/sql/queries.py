...
all_queries = [
        {
            ...
            "query": """
                ...
                JOIN 
                    competitionscleaned C ON CT.CompetitionId = C.Id
                JOIN 
                    userachievements UA ON UA.UserId = C.Id
                GROUP BY 
                    T.Name
                ORDER BY 
                    TotalMedals DESC, ActiveUsers DESC
                LIMIT 10;
            """
        },
        {
            ...
            "query": """
                ...
                FROM 
                    FollowerCounts FC
                JOIN 
                    users U ON FC.UserId = U.Id
                LEFT JOIN 
                    userachievements UA ON U.Id = UA.UserId
                GROUP BY 
                    U.DisplayName, FC.FollowerCount
                ORDER BY 
                    FC.FollowerCount DESC, TotalMedals DESC
                LIMIT 10;
            """
        },
        {
            ...
            "query": """ 
                ...
                JOIN 
                    userachievements UA ON U.Id = UA.UserId
                JOIN 
                    submissionscleaned S ON U.Id = S.SubmittedUserId
                JOIN 
                    competitionscleaned C ON S.TeamId = C.Id
                JOIN 
                    competitiontags CT ON C.Id = CT.CompetitionId
                JOIN 
                    tags T ON CT.TagId = T.Id
                JOIN 
                    datasetscleaned DC ON U.Id = DC.CreatorUserId
                GROUP BY 
                    U.DisplayName, T.Name
                HAVING 
                    SUM(UA.TotalGold + UA.TotalSilver + UA.TotalBronze) > 1  -- Users with more than one medal
                    AND COUNT(DISTINCT DC.Id) > 1                            -- Users with more than one dataset
                ORDER BY 
                    TotalMedals DESC, AvgDatasetViews DESC;
        ...
        },
        {
            ...
            "query": """
                ...
                JOIN 
                    competitionscleaned C ON CT.CompetitionId = C.Id
                JOIN 
                    submissionscleaned SC ON C.Id = SC.TeamId
                GROUP BY 
                    T.Name
                HAVING 
                    COUNT(SC.Id) > 100
        ...
        },
        {
            ...
            JOIN 
                datasetscleaned DC ON U.Id = DC.CreatorUserId
            JOIN 
                datasettags DT ON DC.Id = DT.DatasetId
            JOIN 
                tags T ON DT.TagId = T.Id
            WHERE 
                UA.TotalGold + UA.TotalSilver + UA.TotalBronze >= 5  -- Only consider users with 5 or more medals
            GROUP BY 
                T.Name
            HAVING 
                COUNT(DISTINCT DC.Id) > 3  -- Only include tags with more than 3 datasets
            ORDER BY 
                AvgViews DESC, AvgDownloads DESC, AvgVotes DESC;
                        """
        }
    ]
...
index_queries = [
        ...
        # Indexes for Query 3
        "CREATE INDEX idx_datasetscleaned_creatoruserid ON datasetscleaned (CreatorUserId);",
        "CREATE INDEX idx_submissionscleaned_submitteduserid_teamid ON submissionscleaned (SubmittedUserId, TeamId);",
        ...
        # Indexes for Query 5
        "CREATE INDEX idx_userachievements_userid_highmedals ON userachievements (UserId, TotalGold, TotalSilver, TotalBronze);",
